...
jQuery(document).ready(function () {
    ...
    jQuery(document).on('click', '#contactform input.buyButtonOkForm', function () {
        if(jQuery('#contactform input[name]').val() != "" &&
            jQuery('#contactform input[name=txtphone]').val() != "")
        {
            jQuery('.popup, .overlay').css({'opacity': '0', 'visibility': 'hidden'});

            jQuery("body").prepend("<div class='oneClick-message'><p>Cпасибо за покупку! Менеджер свяжется с Вами в ближайшее время.</p><p>Щелкните по экрану, чтобы закрыть это сообщение</p></div>");
            jQuery(".oneClick-message")
                .animate({top: "0"}, 600)
                .click(function(){
                    $(this).animate({top: "-10em"}, 600);
                    $(this).remove();
                });
        }
    });
