		...
		function wcct_cart_button() {
			...
			<a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'woocommerce-cart-tab' ); ?>" class="cart-parent <?php echo esc_attr( $visibility ); ?>">
				<?php
					...
					echo '<div class="cart-title">Корзина</div>';
					echo '<span class="contents">' . sprintf( _n( 'Товаров:  %d', 'Товаров:  %d', intval( $woocommerce->cart->get_cart_contents_count() ), 'woocommerce-cart-tab' ), intval( $woocommerce->cart->get_cart_contents_count() ) ) . '</span>';
